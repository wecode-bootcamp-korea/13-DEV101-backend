...
class SelectPackageTest(TestCase):
    def setUp(self):
        Category.objects.create(
            id=1,
            name='category1'
    ...
    def test_product_package_get_success(self):
        client=Client()
        self.maxDiff = None
        response=client.get('/product/1/packages')
        self.assertEqual(response.status_code,200)
        self.assertEqual(response.json(),
            {
                "product_id": 1,
    ...
    def test_product_package_get_fail(self):
        client=Client()
        response=client.get('/product/9999/packages')
        self.assertEqual(response.status_code,400)
        self.assertEqual(response.json(),
            {
    ...
    def test_product_package_get_not_found(self):
        client=Client()
        response=client.get('/product/1/package')
        self.assertEqual(response.status_code,404)
